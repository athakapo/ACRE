...
def call_experiment(exp_name, thunk, seed=0, num_cpu=1, data_dir=None, 
    ...
    def thunk_plus():
        ...
        if 'suite' in kwargs and 'env_name' in kwargs:
            env_name = kwargs['env_name']
            if kwargs['suite'] == 'dm_control':
                import dmc2gym
                env_task = kwargs['env_task']
                kwargs['env_fn'] = lambda: dmc2gym.make(domain_name=env_name, task_name=env_task)
                del kwargs['env_task']
            else:
                import gym
