...
def ppo_gmm(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), reward_type=None, plot_gmm=False,
    ...
    # Count variables
    var_counts = tuple(core.count_vars(module) for module in [ac.pi, ac.v_core, ac.v_ex, ac.v_in])
    logger.log('\nNumber of parameters: \t pi: %d, \t v_core: %d, \t v_ex: %d, \t v_in: %d' % var_counts)
    ...
    def compute_loss_v(data, intrinsic_update_flag):
    ...
    def update():
        ...
        v_l_old = compute_loss_v(data, gmm.trained).item()
    ...
    for epoch in range(epochs):
        ...
        logger.log_tabular('LogProbReward', average_only=True)
