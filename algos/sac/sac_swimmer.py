...
if __name__ == '__main__':
    ...
    parser.add_argument('--task', type=str, default='swimmer6')
    ...
    if args.tensorboard:
        if args.reward_type is not None:
            instance_details = f"{args.env}_{args.task}-RT{args.reward_type}-{args.exp_name}-[{args.l}_{args.hid}]"
        else:
            instance_details = f"{args.env}_{args.task}-{args.exp_name}-[{args.l}_{args.hid}]"
        logger_tb_args['instance_details'] = instance_details
        logger_tb_args['aggregate_stats'] = args.aggregate_stats

    torch.set_num_threads(torch.get_num_threads())

    sac(lambda: dmc2gym.make(domain_name=args.env, task_name=args.task, seed=args.seed), actor_critic=core.MLPActorCritic,
