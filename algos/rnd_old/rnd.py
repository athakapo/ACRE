...
from algos.rnd.core import MLPRND
...
class RNDBuffer:
    ...
    def __init__(self, obs_dim, act_dim, size, gamma=0.99, lam=0.95):
        ...
        self.combined_adv = np.zeros(size, dtype=np.float32)
    ...
    def store(self, obs, act, rew, intr, val, val_intr, logp):
        """
    ...
    def finish_path(self, last_val=0, last_val_intr=0):
        ...
        #TODO check last_val_intr here | what is the appropriate value
        intrs = np.append(self.intr_buf[path_slice], last_val_intr)
        # the next two lines implement the intrinsic rewards normalization
        intrs_mean, intrs_std = mpi_statistics_scalar(intrs)
        intrs = (intrs - intrs_mean) / intrs_std

        vals_intrs = np.append(self.val_intr_buf[path_slice], last_val_intr)
        deltas_intrs = intrs[:-1] + self.gamma * vals_intrs[1:] - vals_intrs[:-1]
    ...
    def get(self):
        ...
        final_adv = np.add(self.adv_buf, self.adv_intr_buf)

        # the next two lines implement the advantage normalization trick
        adv_mean, adv_std = mpi_statistics_scalar(final_adv)
        final_adv = (final_adv - adv_mean) / adv_std

        data = dict(obs=self.obs_buf, act=self.act_buf, ret=self.ret_buf, ret_intr = self.ret_intr_buf,
                    adv=final_adv, logp=self.logp_buf)
...
def rnd(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), reward_type=None,
    ...
    #hyperparameter
    output_space = 20
    intrinsic_agent = MLPRND(obs_dim[0], output_space, **ac_kwargs)
    ...
    buf = RNDBuffer(obs_dim, act_dim, local_steps_per_epoch, gamma, lam)
    ...
    for epoch in range(epochs):
        for t in range(local_steps_per_epoch):
            ...
            r_i = 20*random.random() #TODO change me
            v_r_i = random.random()
            ep_ret += true_r
            ep_len += 1

            # save and log
            buf.store(o, a, r, r_i, v, v_r_i, logp)
            ...
            if terminal or epoch_ended:
                ...
                if timeout or epoch_ended:
                    ...
                    v = 0
                buf.finish_path(v, random.random())
                if terminal:
